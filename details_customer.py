...
def wizard_form_body():
    if(st.session_state['current_step']) == 1:
        ...
        with col1:
            cag = st.selectbox( 'Customer Account Group', list_column('Customer Account Group'),index=default_value_index(list_column('Customer Account Group'), "cag"))
...
def wizard_form_footer():    
    ...
    with form_footer_container.container():
        ...
        submit = form_footer_cols[3].button('Submit', disabled=disable_submit_button)
        if submit:
            st.write('Form Submitted')
            if 'edited_df_bank' in st.session_state:
                bank_info = st.session_state.edited_df_bank.to_dict('records')
            if 'edited_df_contact' in st.session_state:
                contact_info = st.session_state.edited_df_contact.to_dict('records')
